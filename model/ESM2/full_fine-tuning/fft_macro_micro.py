...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    """
    The main function to run the ESM-2 FFT benchmark for PrMFTP.
    """
    print(f"\n===== Running ESM-2 FFT Benchmark for PrMFTP =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ESM2_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ESM2_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ESM-2 Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/FFT_ESM2_PrMFTP/ESM2_{args.run_time}"
    ...
    csv_path = f"./results/FFT/ESM2_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ESM-2 FFT Benchmark for PrMFTP (Macro & Micro)")
