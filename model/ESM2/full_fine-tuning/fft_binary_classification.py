...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    print(f"\n===== Running ESM-2 FFT Benchmark for: {dataset_name} =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ESM2/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ESM2/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ESM-2 Full Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # Save the best model
    checkpoint_dir = f"./checkpoint/FFT_ESM2/{dataset_name}/ESM2_{args.run_time}"
    ...
    print(f"===== ESM-2 FFT Benchmark for {dataset_name} Finished =====\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ESM-2 3B Full Fine-Tuning Benchmark")
    parser.add_argument("--dataset", type=str, required=True, choices=["ToxinPred", "HemoPI"],
