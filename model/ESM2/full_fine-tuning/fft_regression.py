...
from transformers import (
    ...
    AutoModel, TrainerCallback, EarlyStoppingCallback,TrainerState,TrainerControl
)
import warnings
...
class ESM2PPIRegressionModel(nn.Module):
    def __init__(self, model_name="./model"):
        super().__init__()
        self.esm2 = AutoModel.from_pretrained(model_name)
        self.dropout = nn.Dropout(0.1)
        self.regressor = nn.Sequential(
            nn.Linear(self.esm2.config.hidden_size * 2, 512),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(512, 128),
            nn.ReLU(),
...
def train_and_evaluate(run_id, model_name="./model", 
                      learning_rate=4e-4, num_epochs=100, batch_size=2):
    
    print(f"Starting ESM2 regression for run_{run_id}")
    
    data_path = "./dataset/PPI/regression"
    output_dir = f'./output/regression_fft/run_{run_id}'
    ...
    model = ESM2PPIRegressionModel(model_name)
    ...
    print(f"ESM2 Regression Run {run_id} Results (Original Scale):")
