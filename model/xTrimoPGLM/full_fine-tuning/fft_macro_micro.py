...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running XTrimoPGLM FFT Benchmark for PrMFTP =====")
    ...
    model = XTrimoPGLMForSequenceClassification(base_model, num_labels=len(PEPTIDE_TYPES))

    training_args = TrainingArguments(
        output_dir=f'./output/FFT_XTrimoPGLM_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_XTrimoPGLM_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting XTrimoPGLM Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/FFT_XTrimoPGLM_PrMFTP/XTrimPGLM_{args.run_time}"
    ...
    csv_path = f"./results/FFT/XTrimoPGLM_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XTrimoPGLM FFT Benchmark for PrMFTP (Macro & Micro)")
