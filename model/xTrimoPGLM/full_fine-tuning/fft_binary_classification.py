...
class XTrimoPGLMForSequenceClassification(torch.nn.Module):
    ...
    def __init__(self, model, num_labels=2):
        super().__init__()
        self.model = model
        # The hidden layer dimension of the model is 2560
...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    model = XTrimoPGLMForSequenceClassification(base_model)

    training_args = TrainingArguments(
        output_dir=f'./output/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting xTrimoOPGLM Full Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # --- Save the best model ---
    checkpoint_dir = f"./checkpoint/{dataset_name}/XTrimPGLM_{args.run_time}"
    ...
    print(f"===== xTrimoPGLM FFT Benchmark for {dataset_name} Finished =====\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="xTrimoPGLM Full Fine-tuning Benchmark")
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
