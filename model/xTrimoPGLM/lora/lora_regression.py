...
from transformers import (
    Trainer, TrainingArguments, AutoTokenizer, AutoModelForMaskedLM,
    AutoConfig, TrainerCallback, EarlyStoppingCallback,TrainerState,TrainerControl
)
import warnings
...
class XTrimoPGLMForSequenceRegression(torch.nn.Module):
    def __init__(self, model, hidden_size=2560):
        super().__init__()
        self.model = model
    ...
    def forward(self, protein_input_ids, protein_attention_mask, 
        ...
        protein_hidden_states = protein_output.hidden_states[:-1]

        protein_cls_embeddings = torch.stack([layer[0, :, :] for layer in protein_hidden_states], dim=1)
        protein_mean_embedding = protein_cls_embeddings.mean(dim=1)
        
        peptide_hidden_states = peptide_output.hidden_states[:-1]

        peptide_cls_embeddings = torch.stack([layer[0, :, :] for layer in peptide_hidden_states], dim=1)
        peptide_mean_embedding = peptide_cls_embeddings.mean(dim=1)
        
        combined_features = torch.cat([protein_mean_embedding, peptide_mean_embedding], dim=1)
...
def train_and_evaluate(run_id, model_name='model', learning_rate=2e-5, num_epochs=100, batch_size=6):

    print(f"Starting xTrimoPGLM regression for run_{run_id}")
    ...
    model = XTrimoPGLMForSequenceRegression(lora_model)
    ...
    print(f"ProtT5 Regression Run {run_id} Results (Original Scale):")
    ...
    results_df = pd.DataFrame({
        ...
        'Predicted_pKd': test_pred_original.flatten()
    })
    
    results_filename = f'./predictions/lora/test_predictions_run_{run_id}.csv'

    os.makedirs(os.path.dirname(results_filename), exist_ok=True)
    
    results_df.to_csv(results_filename, index=False)
    print(f"Test predictions and true values saved to {results_filename}")
