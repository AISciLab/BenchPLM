...
from transformers import (
    ...
    AutoModelForMaskedLM, TrainerCallback
)
...
class XTrimoPGLMForSequenceClassification(torch.nn.Module):
    """
    Custom wrapper for XTrimoPGLM to add a multi-label classification head.
    """
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running XTrimoPGLM LoRA Benchmark for PrMFTP =====")
    ...
    model = XTrimoPGLMForSequenceClassification(peft_model, num_labels=len(PEPTIDE_TYPES))

    training_args = TrainingArguments(
        output_dir=f'./output/Lora_XTrimoPGLM_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/Lora_XTrimoPGLM_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting XTrimoPGLM LoRA Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/Lora_XTrimoPGLM_PrMFTP/XTrimPGLM_{args.run_time}"
    ...
    csv_path = f"./results/Lora/XTrimoPGLM_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XTrimoPGLM LoRA Benchmark for PrMFTP (Macro & Micro)")
    parser.add_argument("--model_name", type=str, default="Bo1015/proteinglm-3b-mlm",
                        help="Path or name of the pre-trained XTrimoPGLM model.")
