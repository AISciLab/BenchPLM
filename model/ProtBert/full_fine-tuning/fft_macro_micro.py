...
class BertForSequenceClassification(torch.nn.Module):
    """
    Custom wrapper for BertModel to add a multi-label classification head.
    """
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running ProtBert FFT Benchmark for PrMFTP =====")
    ...
    model = BertForSequenceClassification(args.model_name, num_labels=len(PEPTIDE_TYPES))

    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ProtBert_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ProtBert_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ProtBert Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/FFT_ProtBert_PrMFTP/ProtBert_{args.run_time}"
    ...
    csv_path = f"./results/FFT/ProtBert_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ProtBert FFT Benchmark for PrMFTP (Macro & Micro)")
