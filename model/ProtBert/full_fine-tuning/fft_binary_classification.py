...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    print(f"\n===== Running ProtBERT FFT Benchmark for: {dataset_name} =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ProtBERT/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ProtBERT/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ProtBERT Full Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # --- Save the best model ---
    checkpoint_dir = f"./checkpoint/FFT_ProtBERT/{dataset_name}/ProtBert_{args.run_time}"
    ...
    print(f"===== ProtBERT FFT Benchmark for {dataset_name} Finished =====\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ProtBERT Full Fine-Tuning Benchmark")
