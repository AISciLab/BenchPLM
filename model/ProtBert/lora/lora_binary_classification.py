...
from transformers import (
    Trainer, TrainingArguments, BertForSequenceClassification, BertTokenizer, TrainerCallback
)
...
def compute_metrics(p):
    ...
    logits, labels = p.predictions[0], p.label_ids
...
# 2. Data Loaders with ProtBERT Preprocessing

def preprocess_for_bert_t5(sequences):
...
def load_toxteller_data(train_path, test_path):
    ...
    train_data = pd.DataFrame({"sequence": preprocess_for_bert_t5(train_seqs), "label": train_labels})
    test_data = pd.DataFrame({"sequence": preprocess_for_bert_t5(test_seqs), "label": test_labels})
    return train_data, test_data


def load_toxinpred_data(train_pos_path, train_neg_path, test_pos_path, test_neg_path):
    """Load the Toxinpred3.0 dataset"""
    ...
    train_data = pd.DataFrame({"sequence": preprocess_for_bert_t5(train_data_raw["sequence"].tolist()),
                               "label": train_data_raw["label"].tolist()})
    test_data = pd.DataFrame({"sequence": preprocess_for_bert_t5(test_data_raw["sequence"].tolist()),
...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    print(f"\n===== Running ProtBERT LoRA Benchmark for: {dataset_name} =====")
    ...
    base_model = BertForSequenceClassification.from_pretrained(args.model_name, num_labels=2)
    model = get_peft_model(base_model, lora_config)
    print("Trainable parameters after applying LoRA:")
    model.print_trainable_parameters()

    training_args = TrainingArguments(
        output_dir=f'./output/Lora_ProtBERT/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/Lora_ProtBERT/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ProtBERT LoRA Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # --- Save the best model ---
    checkpoint_dir = f"./checkpoint/Lora_ProtBERT/{dataset_name}/ProtBert_{args.run_time}"
    ...
    print(f"===== ProtBERT LoRA Benchmark for {dataset_name} Finished =====\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ProtBERT LoRA Fine-Tuning Benchmark")
