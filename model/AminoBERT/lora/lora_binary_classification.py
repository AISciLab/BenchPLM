...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    print(f"\n===== Running AminoBERT LoRA Benchmark for: {dataset_name} =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/Lora_AminoBERT/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/Lora_AminoBERT/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting AminoBERT LoRA Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # Save the best model
    checkpoint_dir = f"./checkpoint/Lora_AminoBERT/{dataset_name}/AminoBert_{args.run_time}"
    ...
    print(f"===== AminoBERT LoRA Benchmark for {dataset_name} Finished =====\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AminoBERT LoRA Fine-Tuning Benchmark")
