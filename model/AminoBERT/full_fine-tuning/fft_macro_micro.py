...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    """
    The main function to run the AminoBERT FFT benchmark for PrMFTP.
    """
    print(f"\n===== Running AminoBERT FFT Benchmark for PrMFTP =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/FFT_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting AminoBERT Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    # Save the best model
    checkpoint_dir = f"./checkpoint/FFT_PrMFTP/AminoBert_{args.run_time}"
    ...
    csv_path = f"./results/FFT/PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AminoBERT FFT Benchmark for PrMFTP (Macro & Micro)")
