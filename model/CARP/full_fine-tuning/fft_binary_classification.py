...
class CARPFinetuneModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.carp_model = self.carp_model
...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    print(f"\n===== Running CARP FFT Benchmark for: {dataset_name} =====")
    ...
    collator = CARPCollator(model.collator)

    training_args = TrainingArguments(
        output_dir=f'./output/FFT_CARP/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_CARP/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
    ...
    print("--- Starting CARP Full Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # Save the best model
    checkpoint_dir = f"./checkpoint/FFT_CARP/{dataset_name}/CARP_{args.run_time}"
    ...
    print(f"===== CARP FFT Benchmark for {dataset_name} Finished =====\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CARP 640M Full Fine-Tuning Benchmark")
    parser.add_argument("--dataset", type_str, required=True, choices=["ToxTeller", "ToxinPred", "HemoPI"],
