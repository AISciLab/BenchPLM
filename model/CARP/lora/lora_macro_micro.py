...
class CARPFinetuneModel(nn.Module):
    """
    The core CARP model with a LoRA adapter and a classification head.
    """
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    """
    The main function to run the CARP LoRA benchmark for PrMFTP.
    """
    print(f"\n===== Running CARP LoRA Benchmark for PrMFTP =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/Lora_CARP_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/Lora_CARP_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
    ...
    print("--- Starting CARP LoRA Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/Lora_CARP_PrMFTP/CARP_{args.run_time}"
    ...
    csv_path = f"./results/Lora/CARP_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CARP LoRA Benchmark for PrMFTP (Macro & Micro)")
