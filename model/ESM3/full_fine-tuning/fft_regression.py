...
class ESMCollator:
    ...
    def __call__(self, batch):
        ...
        protein_input_ids = torch.full((len(protein_tokens), protein_max_len), 1, dtype=torch.long)  # 1是padding index
        ...
        peptide_input_ids = torch.full((len(peptide_tokens), peptide_max_len), 1, dtype=torch.long)  # 1是padding index
...
def train_and_evaluate(run_id, learning_rate=2e-5, num_epochs=100, batch_size=3):
