...
class ESM3FinetuneModel(nn.Module):
    """
    The core ESM-3 model with a classification head for full fine-tuning.
    """
...
class ESMCollator:
    """
    Custom data collator for the ESM-3 model.
    """
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    """
    The main function to run the ESM-3 FFT benchmark for PrMFTP.
    """
    print(f"\n===== Running ESM-3 FFT Benchmark for PrMFTP =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ESM3_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ESM3_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
    ...
    print("--- Starting ESM-3 Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/FFT_ESM3_PrMFTP/ESM3_{args.run_time}"
    ...
    csv_path = f"./results/FFT/ESM3_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ESM-3 FFT Benchmark for PrMFTP (Macro & Micro)")
