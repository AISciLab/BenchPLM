...
from transformers import (
    ...
    T5EncoderModel, TrainerCallback, EarlyStoppingCallback,TrainerState,TrainerControl
)
import warnings
import json
...
class ProtT5PPIRegressionModel(nn.Module):
    def __init__(self, model_name='model/prot_t5_xl_uniref50'):
        super().__init__()
        base_model = T5EncoderModel.from_pretrained(model_name,).to("cpu")
        ...
        self.prot_t5 = get_peft_model(base_model, peft_config)
        print("Trainable parameters after applying LoRA:")
        self.prot_t5.print_trainable_parameters()
        
        self.dropout = nn.Dropout(0.1)
        self.regressor = nn.Sequential(
            nn.Linear(self.prot_t5.config.d_model * 2, 512),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(512, 128),
            nn.ReLU(),
    ...
    def forward(self, protein_input_ids, protein_attention_mask, 
        ...
        protein_outputs = self.prot_t5(
            input_ids=protein_input_ids,
            attention_mask=protein_attention_mask
        )

        protein_features = protein_outputs.last_hidden_state.mean(dim=1)
        
        peptide_outputs = self.prot_t5(
            input_ids=peptide_input_ids,
            attention_mask=peptide_attention_mask
        )
        peptide_features = peptide_outputs.last_hidden_state.mean(dim=1)
...
def train_and_evaluate(run_id, model_name='model/prot_t5_xl_uniref50', 
                      learning_rate=3e-4, num_epochs=100, batch_size=8):
    print(f"Starting ProtT5 regression for run_{run_id}")
    ...
    model = ProtT5PPIRegressionModel(model_name)
