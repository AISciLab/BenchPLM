...
class T5ForSequenceClassification(torch.nn.Module):
    """
    Custom wrapper for a (potentially LoRA-adapted) T5EncoderModel to add a multi-label classification head.
    """
...
def load_prmftp_data(train_path, test_path):
    ...
    # ProtT5 specific pre-processing
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running ProtT5 LoRA Benchmark for PrMFTP =====")
    ...
    encoder = T5EncoderModel.from_pretrained(args.model_name)
    ...
    peft_encoder = get_peft_model(encoder, peft_config)
    print("Trainable parameters after applying LoRA:")
    peft_encoder.print_trainable_parameters()

    model = T5ForSequenceClassification(peft_encoder, num_labels=len(PEPTIDE_TYPES))

    training_args = TrainingArguments(
        output_dir=f'./output/Lora_ProtT5_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/Lora_ProtT5_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ProtT5 LoRA Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/Lora_ProtT5_PrMFTP/ProtT5_{args.run_time}"
    ...
    csv_path = f"./results/Lora/ProtT5_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ProtT5 LoRA Benchmark for PrMFTP (Macro & Micro)")
    parser.add_argument("--model_name", type=str, default="Rostlab/prot_t5_xl_uniref50",
                        help="Path to the pre-trained ProtT5 model.")
