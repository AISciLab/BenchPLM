...
class T5ForSequenceClassification(torch.nn.Module):
    """
    Custom wrapper for T5EncoderModel to add a multi-label classification head.
    """
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        # The Trainer expects a tuple (loss, logits)
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running ProtT5 FFT Benchmark for PrMFTP =====")
    ...
    model = T5ForSequenceClassification(args.model_name, num_labels=len(PEPTIDE_TYPES))

    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ProtT5_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ProtT5_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ProtT5 Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/FFT_ProtT5_PrMFTP/ProtT5_{args.run_time}"
    ...
    csv_path = f"./results/FFT/ProtT5_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ProtT5 FFT Benchmark for PrMFTP (Macro & Micro)")
