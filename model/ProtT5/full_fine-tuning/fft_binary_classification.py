...
def preprocess_for_prott5(sequences):
    """Pre-process the sequence as required by ProtT5。"""
...
def load_toxinpred_data(train_pos_path, train_neg_path, test_pos_path, test_neg_path):
    ...
    train_data = pd.DataFrame({
        "sequence": preprocess_for_prott5(train_data_raw["sequence"].tolist()),
        "label": train_data_raw["label"].tolist()
    })
    test_data = pd.DataFrame({
        "sequence": preprocess_for_prott5(test_data_raw["sequence"].tolist()),
...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    ...
    print(f"\n===== Running ProtT5 FFT Benchmark for: {dataset_name} =====")
    ...
    model = T5ForSequenceClassification(base_model)

    training_args = TrainingArguments(
        output_dir=f'./output/FFT_ProtT5/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_ProtT5/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting ProtT5 Full Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    # --- Save the best model ---
    checkpoint_dir = f"./checkpoint/FFT_ProtT5/{dataset_name}/ProtT5_{args.run_time}"
    ...
    print(f"===== ProtT5 FFT Benchmark for {dataset_name} Finished =====\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ProtT5 Full Fine-Tuning Benchmark")
