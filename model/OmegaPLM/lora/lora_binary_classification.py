...
os.environ['INFRA_PROVIDER'] = 'omegafold'  # OmegaFold requires this environment variable
import argparse
import pandas as pd
import numpy as np
...
class OmegaCollator:
    ...
    def __init__(self, pad_token_id=21):  # 21 is typically used for padding/unknown
...
def run_benchmark(dataset_name: str, train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running OmegaPLM LoRA Benchmark for: {dataset_name} =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/Lora_OmegaPLM/{dataset_name}/Run_results_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/Lora_OmegaPLM/{dataset_name}/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
        report_to="none"
    )
    ...
    print("--- Starting OmegaPLM LoRA Fine-Tuning ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/Lora_OmegaPLM/{dataset_name}/OmegaPLM_{args.run_time}"
    trainer.save_model(checkpoint_dir)
    print(f"Best model saved to {checkpoint_dir}")  # Note: No tokenizer to save
    ...
    print(f"===== OmegaPLM LoRA Benchmark for {dataset_name} Finished =====\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OmegaPLM LoRA Fine-Tuning Benchmark")
