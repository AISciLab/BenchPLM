...
class OmegaForSequenceClassification(torch.nn.Module):
    """
    Custom module for OmegaFold sequence classification.
    """
...
class OmegaCollator:
    """
    Custom data collator for the OmegaFold model.
    """
...
def tokenize_sequence(sequence):
    """Converts a protein sequence string to a list of integer token IDs."""
    return [rc.restypes_with_x.index(aa) if aa in rc.restypes_with_x else 21 for aa in sequence]


def encode_data(batch):
    """Encodes a batch of sequences for the Dataset object."""
    return {
...
def run_benchmark(train_data: pd.DataFrame, test_data: pd.DataFrame, args):
    print(f"\n===== Running OmegaFold FFT Benchmark for PrMFTP =====")
    ...
    training_args = TrainingArguments(
        output_dir=f'./output/FFT_OmegaFold_PrMFTP/Run_{args.run_time}',
        ...
        save_total_limit=1,
        logging_dir=f"./logs/FFT_OmegaFold_PrMFTP/run_{args.run_time}",
        logging_steps=100,
        bf16=True,
    ...
    print("--- Starting OmegaFold Full Fine-Tuning for PrMFTP ---")
    trainer.train()
    print("--- Training Finished ---")

    checkpoint_dir = f"./checkpoint/FFT_OmegaFold_PrMFTP/OmegaPLM_{args.run_time}"
    ...
    csv_path = f"./results/FFT/OmegaFold_PrMFTP_micro_results_run_{args.run_time}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OmegaFold FFT Benchmark for PrMFTP (Macro & Micro)")
